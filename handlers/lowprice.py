...
from hotel_requests import get_id_city, get_hotel_info_cheap, send_hotels_without_photo, set_hotels_with_photos
...
async def result(message: types.Message, state: FSMContext):
    ...
    if 'Нет' in answer:
        ...
        logger.info("Бот приступает к выполнению команды /lowprice")
        try:
            data = await state.get_data()
            days_quantity = (data['departure_date'] - data['date_of_entry']).days
            city_id = await get_id_city(data)
            hotels_information = await get_hotel_info_cheap(data, city_id)
